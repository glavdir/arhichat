...
def text(message,chat_socket=''):
    ...
    for new_shout in new_shouts:
        news = shouts.do_newshout(new_shout.get('s_user'), new_shout.get('msg'), new_shout.get('s_me'), new_shout.get('s_private'))
    ...
    if doshout:
        ...
        if newshout.s_private == -1:
            try:
                iserttext = "insert into Corvus (s_user, s_time, s_shout, s_me, s_private) values (:s_user, :s_time, :s_shout, :s_me, :s_private)"
                db.session.execute(iserttext, {'s_user':newshout.s_user, 's_time':newshout.s_time, 's_shout':newshout.s_shout, 's_me':newshout.s_me, 's_private':newshout.s_private})
                db.session.commit()
            except:
